...
const TaskDetails = () => {
    ...
    const [hasCommentsAuthError, setHasCommentsAuthError] = useState(false);

    const { currentWorkspace } = useSelector((state) => state.workspace);

    const fetchComments = async () => {
        if (!taskId || hasCommentsAuthError) {
            return;
        }

        try {
            const token = await getToken()
            const {data} = await api.get(`/comments/${taskId}` , {headers : {Authorization : `Bearer ${token}`}})
            setComments(data.comments || [])
        } catch (error) {
            // If auth/session expires while on the page, stop polling
            // and show a single clear message instead of spamming toasts.
            if (error?.response?.status === 401) {
                if (!hasCommentsAuthError) {
                    setHasCommentsAuthError(true);
                    toast.error("Session expired. Please refresh and sign in again to see new comments.");
                }
                return;
            }
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            const token = await getToken()
            const {data} = await api.post(`/comments` , {taskId : task.id , content: newComment} , {headers : {Authorization : `Bearer ${token}`}})
    ...
    useEffect(() => {
        if (taskId && task && !hasCommentsAuthError) {
            fetchComments();
            const interval = setInterval(() => { fetchComments(); }, 10000);
            return () => clearInterval(interval);
        }
    }, [taskId, task, hasCommentsAuthError]);
