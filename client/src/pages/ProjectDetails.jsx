...
export default function ProjectDetail() {
    ...
    // Find the current user's DB record inside workspace members
    const currentWorkspace = useSelector((state) => state?.workspace?.currentWorkspace);
    const currentUserDbId = currentWorkspace?.members?.find((m) => m.user?.email === user?.primaryEmailAddress?.emailAddress)?.user?.id;
                ...
                <div className="mt-6">
                    {activeTab === "tasks" && (
                        <div className=" dark:bg-zinc-900/40 rounded max-w-6xl">
                            <ProjectTasks tasks={tasks} project={project} currentUserDbId={currentUserDbId} />
                        </div>
                    )}
                    {activeTab === "analytics" && (
                        <div className=" dark:bg-zinc-900/40 rounded max-w-6xl">
