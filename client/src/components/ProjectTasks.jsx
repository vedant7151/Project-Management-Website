...
const ProjectTasks = ({ tasks, project, currentUserDbId }) => {
    const {getToken} = useAuth()
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [selectedTasks, setSelectedTasks] = useState([]);

    // Check if current user is the team lead of this project
    const isTeamLead = project?.team_lead === currentUserDbId;
    ...
    const handleDelete = async () => {
        try {
            ...
            //  Simulate API call
            await api.post("/tasks/delete" , {taskId : selectedTasks} , {headers : {Authorization : `Bearer ${token}`}})
                                            ...
                                            <tr key={task.id} onClick={() => navigate(`/taskDetails?projectId=${task.projectId}&taskId=${task.id}`)} className=" border-t border-zinc-300 dark:border-zinc-800 group hover:bg-zinc-50 dark:hover:bg-zinc-700 transition-all cursor-pointer" >
                                                ...
                                                <td onClick={e => e.stopPropagation()} className="px-4 py-2">
                                                    {isTeamLead ? (
                                                        ...
                                                        <span title="Only the team lead can update task status" className={`text-xs px-2 py-1 rounded cursor-not-allowed ${
                                    ...
                                    <div key={task.id} className=" dark:bg-gradient-to-br dark:from-zinc-800/70 dark:to-zinc-900/50 border border-zinc-300 dark:border-zinc-800 rounded-lg p-4 flex flex-col gap-2">
                                        ...
                                        <div>
                                            ...
                                            {isTeamLead ? (
                                                ...
                                                <div title="Only the team lead can update task status" className={`w-full mt-1 px-2 py-1 rounded text-sm text-center cursor-not-allowed ${
                                                    task.status === 'DONE' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400' :
                                                    task.status === 'IN_PROGRESS' ? 'bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-400' :
                                                    'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400'
                                                }`}>
